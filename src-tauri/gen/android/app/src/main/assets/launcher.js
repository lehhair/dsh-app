(() => {
  ...
  function setState(status, port, url) {
    ...
    meta.textContent = status === "running" ? `http://127.0.0.1:${port}/` : status === "starting" ? "\u6B63\u5728\u542F\u52A8\u2026" : "\u672A\u542F\u52A8";
...
${(await bridge.logs().catch(() => [])).join("\n").slice(-4e3)}`;
  ...
  var remoteList = document.getElementById("remote-list");
  ...
  var remoteSearch = document.getElementById("remote-search");
  var remoteError = document.getElementById("remote-error");
  var restoreToggle = document.getElementById("restore-toggle");
  var autoLocalToggle = document.getElementById("auto-local-toggle");
  var healthCache = /* @__PURE__ */ new Map();
  async function renderRestore() {
    const on2 = await bridge.settings.getRestore();
    restoreToggle.classList.toggle("on", on2);
    restoreToggle.setAttribute("aria-checked", String(on2));
  }
  restoreToggle.addEventListener("click", async () => {
    const on2 = !restoreToggle.classList.contains("on");
    await bridge.settings.setRestore(on2);
    restoreToggle.classList.toggle("on", on2);
    restoreToggle.setAttribute("aria-checked", String(on2));
  });
  async function renderAutoLocal() {
    const on2 = await bridge.settings.getAutoLocal();
    autoLocalToggle.classList.toggle("on", on2);
    autoLocalToggle.setAttribute("aria-checked", String(on2));
  }
  autoLocalToggle.addEventListener("click", async () => {
    const on2 = !autoLocalToggle.classList.contains("on");
    await bridge.settings.setAutoLocal(on2);
    autoLocalToggle.classList.toggle("on", on2);
    autoLocalToggle.setAttribute("aria-checked", String(on2));
  });
  var dshVersionEl = document.getElementById("dsh-version");
  var checkUpdateBtn = document.getElementById("check-update");
  var doUpdateBtn = document.getElementById("do-update");
  ...
  var launcherUpdateRow = document.getElementById("launcher-update-row");
  var launcherUpdateLabel = document.getElementById("launcher-update-label");
  var launcherUpdateBtn = document.getElementById("launcher-update");
  ...
  function applyHealth(dot, status) {
    dot.className = status === "online" ? "dot done" : status === "unauthorized" ? "dot danger" : "dot";
    dot.title = status === "online" ? "\u5728\u7EBF" : status === "unauthorized" ? "\u5BC6\u94A5\u65E0\u6548" : "\u79BB\u7EBF";
  }
  ...
  async function renderRemoteList() {
    ...
    const matches = query ? instances.filter((inst) => inst.name.toLowerCase().includes(query) || inst.url.toLowerCase().includes(query)) : instances;
    for (const inst of matches) {
      ...
      meta2.textContent = `${inst.url} \xB7 \u5BC6\u94A5${inst.keyConfigured ? "\u5DF2\u914D\u7F6E" : "\u672A\u914D\u7F6E"}`;
  ...
  bridge.appInfo().then((info) => {
    ...
    if (!info.desktop) {
      ...
      if (sub) sub.textContent = "\u79FB\u52A8\u5BA2\u6237\u7AEF \xB7 \u8FDE\u63A5\u8FDC\u7A0B dsh \u8282\u70B9\uFF08\u7ECF\u7F51\u5173\uFF09";
  ...
  var syncActive = () => document.body.classList.toggle("inactive", !document.hasFocus());
